...
Eventは、_Event Type_と呼ぶ項目を使って定義します。
Events are defined using an item called an .
イベント型は送信者によるスキーマと
The Event Type allows events to have their structure declared with a schema by
...
[#196]
== {MUST} EventスキーマはOpen APIスキーマオブジェクトに準拠する

API仕様にEventスキーマ仕様を揃えるために、私たちはEventスキーマの定義にも
...
最後に、JSONスキーマのいくつかのキーワードを拡張したものがあります。
...
[#214]
== {MUST} 重複したイベントPrepare for duplicate Events
